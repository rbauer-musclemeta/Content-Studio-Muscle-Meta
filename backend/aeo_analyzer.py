...
def analyze_aeo(
    ...
    raw = content  # Keep raw for HTML heading analysis
...
def _analyze_headings(content: str, is_html: bool) -> Dict[str, Any]:
    ...
    # Deep structure (H3/H4) preferred by AI engines
    depth_count = h3s + h4s
    if depth_count >= 5:
        score += 40
...
def _analyze_title(title: str, keywords: Optional[List[str]]) -> Dict[str, Any]:
    ...
    # Check for high-performing title patterns
    ...
    # Year in title (current year preferred)
    ...
    if keywords:
        ...
        score += 10  # Can't assess without keywords

    # Title length (50-70 chars optimal for SERP display)
    tlen = len(title)
    if 50 <= tlen <= 70:
        score += 20
...
def _analyze_structure(text: str, raw: str, is_html: bool) -> Dict[str, Any]:
    ...
    # Lists (bullet/numbered) — AI engines prefer lists
    ...
    # Word count (1500-3000 sweet spot for comprehensive coverage)
...
def _analyze_authority(text: str) -> Dict[str, Any]:
    ...
    # Citation count (PMID, DOI patterns)
    ...
    # Statistics (percentages, numbers with context)
    stat_patterns = [
        r"\d+(?:\.\d+)?%",
        r"\d+(?:,\d{3})+",
    ...
    # Study type mentions (meta-analysis, RCT, systematic review)
    high_evidence = [
    ...
    if evidence_count >= 2:
        score += 20
        notes.append(f"High-evidence study types mentioned: Strong for medical content")
...
def _assess_platform_fit(title: str, text: str, heading: Dict) -> Dict[str, Dict[str, Any]]:
    ...
    # Platform preferences from the AEO briefing
    platforms = {
        "google_ai_overview": {
